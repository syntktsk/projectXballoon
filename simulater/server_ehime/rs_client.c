...
//                      ver 1.1       T. Yoshikawa  (2025.12.15)
//    ☆SERVERのIPをRAS1のIPに設定した
//
//
...
#include "elmo_rs_structure_v2p0.h"
//構造体ess1とess2の定義に使用
//ess1はElmoドライバのstatusを格納
//ess2はGNSSコンパスのstatusを格納

#define SERVER_IP "192.168.100.201"    //本館5FにあるRAS1をSERVERとした
#define PORT 9998
#define BUFF 1024
...
void print_ess1(const ess1 *p) {
    printf("[recv] ess1:\n");
    printf("MO=%d UM=%d MF=%.2f EC=%d PX=%d VX=%.2f IQ=%.2f ID=%.2f MC=%.2f BV=%d TI=%d TC=%.2f JV=%.2f PA=%d PR=%d ac=%d ef=%d ps=%d lc=%d\n",
        p->MO, p->UM, p->MF, p->EC, p->PX, p->VX, p->IQ, p->ID, p->MC,
        p->BV, p->TI, p->TC, p->JV, p->PA, p->PR, p->ac, p->ef, p->ps, p->lc);
...
int main() {
    ...
    while (1) {
        ...
        scanf("%32s", send_buf);  // 入力（スペース区切りの文字列を受け取る）
    ...
    //構造体処理
        ...
        if (strncmp(send_buf, "rs", 2) == 0) {//ここはよくないかも？　クライアントが打った文字じゃなくて、愛媛大学のRas(サーバ)から帰ってきたデータでif文を作りたい。 // "rs" の後ろを int に変換 int value = atoi(msg + 2);
                ...
                //rs3を打った時、ess1とess2を受け取る
                else if (strcmp(send_buf, "rs3") == 0) {
                    if (recv_len == sizeof(ess1)) print_ess1((ess1 *)recv_buf);
                    recv_len = recvfrom(sock, recv_buf, sizeof(recv_buf), 0, NULL, NULL);//ess2の受け取り用に、recvfromしている
                    if (recv_len == sizeof(ess2)) print_ess2((ess2 *)recv_buf);
                }
