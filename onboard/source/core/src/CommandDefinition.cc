...
bool CommandDefinition::setCommand(const std::vector<uint8_t>& v)
{
  ...
  // Header (0xEB90) と Footer (0xC5A4) のチェック
  if (v[0] != 0xeb || v[1] != 0x90) {
    std::cerr << "start code incorrect" << std::endl;
    return false;
  }
  ...
  uint16_t crc_calc = calcCRC16(com_without_footer);
  ...
  if (crc_calc != crc_attached) {
    ...
    std::cerr << "Calculated: " << std::hex << crc_calc << ", Attached: " << crc_attached << std::dec << std::endl;
    return false;
  }
...
void CommandDefinition::interpret()
{
  ...
  for (size_t i = 0; i < DATA_LENGTH; ++i) {
      uint8_t byte_value = command_[HEADER_SIZE + i];
      if (byte_value == '\0') break; 
      arg_string += static_cast<char>(byte_value);
  }
  arguments_.push_back(arg_string);
}
