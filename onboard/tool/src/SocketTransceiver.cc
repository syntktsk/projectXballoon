...
bool SocketTransceiver::open_socket(const std::string& hostIp, int port) {
    if (is_running_) {
    ...
    // 受信スレッドを起動
    is_running_ = true;
    receiving_thread_ = std::thread(&SocketTransceiver::receiving_thread_func, this);
...
void SocketTransceiver::receiving_thread_func() {
...
bool SocketTransceiver::send(const std::vector<uint8_t>& command) {
