...
ReceiveCommand::ReceiveCommand()
{
  ...
  code_map_["Set"]                  = CommandProperty{500, 0};
  code_map_["Optional"]             = CommandProperty{600, 0};
  code_map_["Other"]                = CommandProperty{900, 0};
}
 
std::string ReceiveCommand::getCommand(const std::vector<uint8_t>& cmd_){
    // 最初に切り離す
    ReceiveCommand::extract_command(cmd_);
    ReceiveCommand::getCommand_string(code_vec_, arg_vec_);
    return StringCommand_;
};


void ReceiveCommand::extract_command(const std::vector<uint8_t>& cmd_) {
...
void ReceiveCommand::getCommand_string(const std::vector<uint8_t>& code_vec, const std::vector<uint8_t>& arg_vec) {
       // EUに送る文字列に変換する
    ...
    if (code >= 100 && code <= 199) {
        ...
        // arg_float = getValue<float>(arg_vec);
        // std::string arg_str = std::to_string(static_cast<int>(arg_float));
        if (code == 501){
            ...
            std::cerr << "ReceiveCommand: command"<< code_int <<" not find" << std::endl;
            return;
        };
